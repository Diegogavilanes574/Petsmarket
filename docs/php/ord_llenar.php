...
for ($i = 0; $i < $array_num; ++$i){
    ...
    $sql = "update orden_examen set estado=2 where id_orden=".$idemp;
    $resp = $bd->query($sql);
...
if(!$resp){
    header('location:../pdf/rpt_orden_examen.php?mn=2');
}else{
    header('location:../pdf/rpt_orden_final.php?orden='.$idemp);
}
